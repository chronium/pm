...
const task = {
  id: 'PM-0050', title: 'Dialog task', track: 'PM', milestone: 'angular-web', priority: 'high',
  prioritySource: 'milestone', prioritySelection: 'inherit', state: 'todo',
  dependencies: { ready: true, dependsOn: [], waitingOn: [], missing: [], summary: 'ready' },
  createdAt: '2026-07-15T00:00:00Z', modifiedAt: '2026-07-15T00:00:00Z', description: '# Body',
  revision: 'revision-1', localMetadata: { filePath: '.pm/tasks/PM-0050.md' },
};
...
describe('TaskApiService', () => {
  ...
  it('sends typed create payloads and captures response ETags', () => {
    ...
    let etag = '';
    api.create({ title: 'New task', track: 'PM', milestone: null, description: 'Body' }).subscribe((response) => etag = api.etag(response));
    ...
    expect(request.request.body).toEqual({ title: 'New task', track: 'PM', milestone: null, description: 'Body' });
  ...
  it('requires the exact current ETag for update, state, and remove mutations', () => {
    const api = TestBed.inject(TaskApiService);
    api.update(task.id, { title: task.title, state: 'review', priority: 'high', description: task.description }, '"revision-1"').subscribe();
    ...
    expect([update, state, remove].every((item) => item.request.headers.get('If-Match') !== '*')).toBe(true);
    update.flush(task); state.flush(task); remove.flush(null, { status: 204, statusText: 'No Content' });
  });

  it('maps problem details, conflicts, not-found, and unavailable responses', () => {
    const api = TestBed.inject(TaskApiService);
    for (const [status, expected] of [[412, true], [404, false], [503, false]] as const) {
      const error = new HttpErrorResponse({ status, error: { title: 'Request failed', detail: `Failure ${status}`, errorCode: 'failure' } });
      expect(api.error(error, 'Fallback')).toEqual({ status, message: `Failure ${status}`, conflict: expected });
    }
