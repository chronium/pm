...
export const wikiPathMatcher: UrlMatcher = (segments) => segments.length
  ? ({ consumed: segments, posParams: { wikiPath: new UrlSegment(segments.map((segment) => segment.path).join('/'), {}) } })
  : null;
...
function matchPrefixedPath(segments: UrlSegment[], prefix: string) {
  return segments.length > 1 && segments[0]!.path === prefix
    ? { consumed: segments, posParams: { wikiPath: new UrlSegment(segments.slice(1).map((segment) => segment.path).join('/'), {}) } }
    : null;
}
