...
describe('wiki tree derivation', () => {
  const summary = (path: string, title = path): WikiPageSummary => ({ path, title, modifiedAt: '2026-01-01T00:00:00Z' });

  it('derives folders from a flat list with deterministic ordering', () => {
    const tree = buildWikiTree([summary('z-last'), summary('architecture/rendering/canvas'), summary('architecture/overview')]);
    expect(tree.map((node) => node.path)).toEqual(['architecture', 'z-last']);
    expect(tree[0]!.children.map((node) => node.path)).toEqual(['architecture/overview', 'architecture/rendering']);
  ...
  it('preserves a page when the same path is also a folder', () => {
    const tree = buildWikiTree([summary('guides', 'Guides home'), summary('guides/start', 'Start')]);
...
describe('WikiStore retained state', () => {
  beforeEach(() => TestBed.configureTestingModule({ providers: [WikiStore, provideHttpClient(), provideHttpClientTesting()] }));
  ...
  it('resolves page/folder coexistence page-first and keeps mutations in the retained index', async () => {
    ...
    store.accept(new HttpResponse({
      body: { path: 'new/page', title: 'New page', createdAt: '2026-01-01T00:00:00Z', modifiedAt: '2026-01-02T00:00:00Z', body: '', revision: 'r2', localMetadata: { filePath: '.pm/wiki/new/page.md' } },
