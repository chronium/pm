...
@Component({
  ...
  template: `
    <section class="wiki-page">
      @if (store.indexLoading()) { <pm-loading-state>Resolving wiki path…</pm-loading-state> }
      @else if (store.indexError()) { <pm-error-state><p>{{ store.indexError() }}</p><button class="pm-button pm-button--secondary" (click)="store.reloadIndex()">Try again</button></pm-error-state> }
      @else if (resolution().kind === 'folder') {
        <pm-wiki-breadcrumbs [path]="wikiPath()" />
        <header class="wiki-page-header"><div><p class="wiki-eyebrow">Folder</p><h1>{{ folderName() }}</h1></div><a class="pm-button pm-button--primary" routerLink="/wiki/new">New page</a></header>
        <div class="wiki-list" aria-label="Pages in folder">
          @for (page of folderPages(); track page.path) {
            <a class="wiki-list-row" [routerLink]="['/wiki', ...page.path.split('/')]"><span class="wiki-list-title">{{ page.title }}</span><code>{{ page.path }}</code><time [attr.datetime]="page.modifiedAt">{{ page.modifiedAt | date:'medium' }}</time></a>
          }
        </div>
      } @else if (resolution().kind === 'missing') {
        <pm-error-state><h1>Page not found</h1><p>No wiki page or folder exists at <code>{{ wikiPath() }}</code>.</p><a class="pm-button pm-button--secondary" routerLink="/wiki">Back to wiki</a></pm-error-state>
...
export class WikiWorkspace {
  ...
  protected readonly folderPages = computed(() => { const value = this.resolution(); return value.kind === 'folder' ? value.pages : []; });
  protected readonly folderName = computed(() => this.wikiPath().split('/').at(-1));

  constructor() { effect(() => { if (this.wikiPath() && this.resolution().kind === 'page') this.store.select(this.wikiPath()); }); }
}
