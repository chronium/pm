...
describe('wiki route matchers', () => {
  ...
  it('requires a path after reserved edit and metadata prefixes', () => {
    ...
    expect(wikiEditMatcher(segments('edit', 'nested', 'page'), null as never, null as never)?.posParams?.['wikiPath']?.path).toBe('nested/page');
    expect(wikiMetaMatcher(segments('meta', 'nested', 'page'), null as never, null as never)?.posParams?.['wikiPath']?.path).toBe('nested/page');
  });
});
