...
Project creation and ID routes require PM signed-request headers. Unknown projects or invalid signatures return `401`. Unknown routes return `404`.
...
PM stores the public Worker project identifier at `.pm/project_id.txt`. Local user identity and the signing private key live in OS user config, outside `.pm/`. `pm init` creates the authenticated Worker project and writes its public identifier.
